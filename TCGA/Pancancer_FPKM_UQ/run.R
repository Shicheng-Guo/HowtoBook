...
ENST<-unlist(lapply(strsplit(as.character(Symbol2$V2),split="[.]"),function(x) x[1]))
match(ENST,Symbol1$transcript_stable_id)
i=18392


for(i in c(4446,8656,23803,22769,21961,4020,18392,7357,1428,30139)){
print(i)
mean<-tapply(as.numeric(input[i,]),Seq,function(x) mean(x,na.rm=T))
sd<-tapply(as.numeric(input[i,]),Seq,function(x) sd(x,na.rm=T))
num<-tapply(as.numeric(input[i,]),Seq,function(x) length(x))
