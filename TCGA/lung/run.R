...
blood<-input[,grep("LAML|DLBC",phen$pid)]
solid<-input[,-grep("LAML|DLBC",phen$pid)]

bidx<-grep("LAML|DLBC",phen$pid)
sidx<-1:ncol(input)[-bidx]
sbio<-apply(input[1:5,],1,function(x) t.test(x[bidx],sidx)$p.value,na.action="na.omit")
