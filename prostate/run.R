...
## Mutation with age
Rlt<-c()
for(i in 1:nrow(sheet9)){
  rlt<-summary(glm(as.numeric(sheet9[i,])~sheet1$Age))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet9)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet9)~sheet1$Age))

## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet9)){
  rlt<-summary(glm(as.numeric(sheet9[i,])~sheet1$fPSA_tPSA))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet9)
hist(log(Rlt[,4],10))
...
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet9)~sheet1$fPSA_tPSA))

## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet9)){
  rlt<-summary(glm(as.numeric(sheet9[i,])~sheet1$Gleason_Score_Value))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet9)
hist(log(Rlt[,4],10))
...
sum(Rlt[,1]<0)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet9)~sheet1$Gleason_Score_Value))
plot(colSums(sheet9)~sheet1$Gleason_Score_Value)
t.test(colSums(sheet9)[which(sheet1$Gleason_Score_Value>7.5)],colSums(sheet9)[which(sheet1$Gleason_Score_Value<7.5)])


## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet9)){
  rlt<-summary(glm(as.numeric(sheet9[i,])~sheet1$Metastasis_binary))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet9)
hist(log(Rlt[,4],10))
...
sum(Rlt[,1]<0)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(glm(colSums(sheet9)~sheet1$Metastasis_binary))
plot(colSums(sheet9)~sheet1$Gleason_Score_Value)
t.test(colSums(sheet9)[which(sheet1$Gleason_Score_Value>7.5)],colSums(sheet9)[which(sheet1$Gleason_Score_Value<7.5)])
...
for(i in 1:nrow(sheet7)){
  rlt<-summary(glm(as.numeric(sheet7[i,])~sheet1$Age))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet7)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet7)~sheet1$Age))

## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet7)){
  rlt<-summary(glm(as.numeric(sheet7[i,])~sheet1$fPSA_tPSA))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet7)
hist(log(Rlt[,4],10))
head(Rlt[order(Rlt[,4]),])
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet7)~sheet1$fPSA_tPSA))

## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet7)){
  rlt<-summary(glm(as.numeric(sheet7[i,])~sheet1$Gleason_Score_Value))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet7)
hist(log(Rlt[,4],10))
...
sum(Rlt[,1]<0)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(lm(colSums(sheet7)~sheet1$Gleason_Score_Value))
plot(colSums(sheet7)~sheet1$Gleason_Score_Value)
t.test(colSums(sheet7)[which(sheet1$Gleason_Score_Value>7.5)],colSums(sheet7)[which(sheet1$Gleason_Score_Value<7.5)])


## Mutation with fPSA/tPSA
Rlt<-c()
for(i in 1:nrow(sheet7)){
  rlt<-summary(glm(as.numeric(sheet7[i,])~sheet1$Metastasis_binary))$coefficients[2,]
  Rlt<-rbind(Rlt,rlt)
}
rownames(Rlt)<-rownames(sheet7)
hist(log(Rlt[,4],10))
...
sum(Rlt[,1]<0)
sum(Rlt[,1]>0)/sum(Rlt[,1]<0)
summary(glm(colSums(sheet7)~sheet1$Metastasis_binary))
plot(colSums(sheet7)~sheet1$Gleason_Score_Value)
t.test(colSums(sheet7)[which(sheet1$Gleason_Score_Value>7.5)],colSums(sheet7)[which(sheet1$Gleason_Score_Value<7.5)])
