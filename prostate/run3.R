...
data<-read.table("symbol.snv.txt",head=T,sep="\t",row.names = 1)
iid<-unlist(lapply(strsplit(as.character(colnames(count)),"[.]"),function(x) x[1]))
sid<-unlist(lapply(strsplit(iid,"_"),function(x) x[1]))
data<-data.matrix(data)
head(data)
data[data == 1]<-0
data[data>0]<-1
...
for(id in unique(sid)){
  ...
  head(input)
  input<-input[-which(rowSums(input)==0),]
  trunk<-names(which(rowSums(input)==3))
  private<-names(which(rowSums(input)==1))
  share<-names(which(rowSums(input)>1 &rowSums(input)<3))
  ReactomePLOT(trunk,paste(id,"trunk",sep="."))
  ReactomePLOT(share,paste(id,"share",sep="."))
  ReactomePLOT(private,paste(id,"private",sep="."))
  }
}

ReactomePLOT(rownames(data),paste("Prostate.whole.mutation",sep="."))
...
gsePathway(, nPerm=100,pvalueCutoff=0.2,pAdjustMethod="BH", verbose=FALSE)
res <- as.data.frame(y)
head(res)
