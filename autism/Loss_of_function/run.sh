...
## identify all loss-of-function SNPs caused by stop_gain and stop_loss (exclude stop_retained_variants)
...
echo cd $(pwd) >> $i.job
echo bcftools view -v snps -f PASS -i \'\(INFO\/vep \~ \"stop_gained\"\|\INFO\/vep \~ \"stop_lost\"\)\' ~/hpc/db/Gnomad/vcf/gnomad.exomes.r2.1.sites.chr$i.rec.vcf.bgz -Ou -o  gnomad.exomes.r2.1.sites.chr$i.rec.$panel.vcf.bgz >>$i.job
